...
const DEFAULT_SEGMENT_FILES = [
  '/data/cibola_phoenix/cibola_to_phoenix_route.geojson',
];

const SOURCE_ID = 'cibola-phoenix-route-source';
const LAYER_ID = 'cibola-phoenix-route-layer';
const PARTICLE_SOURCE_ID = 'cibola-phoenix-route-particles';
const PARTICLE_LAYER_ID = 'cibola-phoenix-route-particles-layer';

export default function CibolaPhoenixRouteLayer({ map, visible }) {
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      try {
        ...
        if (!m.getLayer(LAYER_ID)) {
          m.addLayer({
            ...
            paint: {
              'line-width': 2,
              'line-opacity': 0.9,
              'line-color': '#EF4444', // Red color for Cibola-Phoenix connection
            },
          });
        }
        ...
        // Animate particles
        const count = 80; // More particles for longer route
        const speed = 0.000005; // Slightly slower for longer route
        ...
        console.error('[CibolaPhoenixRouteLayer] Error:', err);
      }
    };
