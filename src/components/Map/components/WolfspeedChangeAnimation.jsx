...
const PERIODS = [
  { id: '2017_2018', label: '2017 → 2018', file: '/data/wolfspeed_nc/wolfspeed_nc_2017_2018.geojson' },
  { id: '2018_2019', label: '2018 → 2019', file: '/data/wolfspeed_nc/wolfspeed_nc_2018_2019.geojson' },
  { id: '2019_2020', label: '2019 → 2020', file: '/data/wolfspeed_nc/wolfspeed_nc_2019_2020.geojson' },
  { id: '2020_2021', label: '2020 → 2021', file: '/data/wolfspeed_nc/wolfspeed_nc_2020_2021.geojson' },
  { id: '2021_2022', label: '2021 → 2022', file: '/data/wolfspeed_nc/wolfspeed_nc_2021_2022.geojson' },
  { id: '2022_2023', label: '2022 → 2023', file: '/data/wolfspeed_nc/wolfspeed_nc_2022_2023.geojson' },
  { id: '2023_2024', label: '2023 → 2024', file: '/data/wolfspeed_nc/wolfspeed_nc_2023_2024.geojson' },
  { id: '2024_2025', label: '2024 → 2025', file: '/data/wolfspeed_nc/wolfspeed_nc_2024_2025.geojson' }
];

const WOLFSPEED_CENTER = [-79.499, 35.717]; // Chatham-Siler City Advanced Manufacturing Site
const WOLFSPEED_RADIUS_METERS = 6000;

const LAYER_IDS = {
  source: 'wolfspeed-geo',
  aoiFill: 'wolfspeed-aoi-fill',
  aoiOuter: 'wolfspeed-aoi-outer',
  industrialFill: 'wolfspeed-industrial-fill',
  roads: 'wolfspeed-roads',
  power: 'wolfspeed-power',
  siteLabel: 'wolfspeed-site-label'
};

const SITE_KEY = 'wolfspeed_nc';

const WOLFSPEED_RING_IDS = {
  circleSource: 'wolfspeed-ring-source',
  haloInnerSource: 'wolfspeed-ring-halo-inner-source',
  haloOuterSource: 'wolfspeed-ring-halo-outer-source',
  sparkleSource: 'wolfspeed-ring-sparkles-source',
  circleFill: 'wolfspeed-ring-fill',
  circleOutline: 'wolfspeed-ring-outline',
  haloInnerLayer: 'wolfspeed-ring-halo-inner',
  haloOuterLayer: 'wolfspeed-ring-halo-outer',
  sparkleLayer: 'wolfspeed-ring-sparkles'
};
...
// Water-aware color ramps (vary by area_ha)
...
const CHANGE_CALLOUTS = {
  ...
  agriculture_gain: { label: 'Vegetation Gain', color: '#38bdf8' },
...
const WolfspeedChangeAnimation = ({ map, data, changeData = null, visible = true }) => {
  ...
  const timelinePlaybackActiveRef = useRef(false);
  ...
  const applyTimelineLegendHighlight = useCallback((seriesKey, fillId, m, lineId = null) => {
    ...
    if (fillId && m.getLayer(fillId)) {
      m.setPaintProperty(fillId, 'fill-extrusion-color',
        match ? ['case', ['==', ['get', 'change_label'], match], WATER_COLOR_RAMP, 'rgba(17,24,39,0.28)']
              : WATER_COLOR_RAMP);
    }
  ...
  useEffect(() => {
    ...
    activeLayersRef.current.forEach(({ fillId, lineId }) => {
      applyTimelineLegendHighlight(timelineHighlightKey, fillId, m, lineId);
    });
  }, [timelineHighlightKey, applyTimelineLegendHighlight, map]);
  ...
  const emitTimelinePause = useCallback(() => {
    if (typeof window === 'undefined' || !window.mapEventBus?.emit) return;
    if (!timelinePlaybackActiveRef.current) return;
    timelinePlaybackActiveRef.current = false;
    window.mapEventBus.emit('timeline:pause', { siteKey: SITE_KEY });
  }, []);

  const emitTimelinePlayback = useCallback((period, index) => {
    if (!period || typeof window === 'undefined' || !window.mapEventBus?.emit) return;
    const annualLabel = period.label || period.yearPair || period.id;
    window.mapEventBus.emit('timeline:playback', {
      siteKey: SITE_KEY,
      periodId: period.id,
      periodLabel: period.label || annualLabel,
      annualLabel,
      periodIndex: index,
      periodCount: periodData.length,
      timestamp: Date.now()
    });
    timelinePlaybackActiveRef.current = true;
  }, [periodData.length]);
  ...
  const addCalloutLayers = useCallback((callouts) => {
    ...
    const sourceId = 'wolfspeed-callouts-source';
    const layerId = 'wolfspeed-callouts-labels';
    ...
    const animate = (now) => {
      ...
      m.setPaintProperty(layerId, 'text-opacity', clamp(eased));
      if (progress < 1) {
  ...
  const animateLayerIn = useCallback((fillId, config) => {
    ...
    const step = (now) => {
      ...
      if (progress < 1) {
        ...
        applyTimelineLegendHighlight(timelineHighlightKey, fillId, m);
  ...
  const animateOutlineGlow = useCallback((outlineId) => {
    ...
    const step = (now) => {
      ...
      if (elapsed >= totalDuration) {
        ...
        applyTimelineLegendHighlight(timelineHighlightKey, null, m, outlineId);
  ...
  const addPeriodToMap = useCallback((period, periodIndex = 0) => {
    ...
    const sourceId = `wolfspeed-change-${baseId}-source`;
    ...
    const baseHeight = 30 * periodIndex;
    ...
    // Apply timeline highlight if active
    applyTimelineLegendHighlight(timelineHighlightKey, fillId, m, outlineId);
  ...
  }, [map, animateLayerIn, animateOutlineGlow, animateShadowIn, addCalloutLayers, removeCalloutLayers, applyTimelineLegendHighlight, timelineHighlightKey]);
  ...
  const addCircleLayer = useCallback(() => {
    ...
    const circleSourceId = 'wolfspeed-site-circle-source';
    const haloInnerSourceId = 'wolfspeed-site-halo-inner-source';
    const haloOuterSourceId = 'wolfspeed-site-halo-outer-source';
    const sparkleSourceId = 'wolfspeed-site-sparkles-source';

    const fillId = 'wolfspeed-site-circle-fill';
    const outlineId = 'wolfspeed-site-circle-outline';
    const haloInnerLayerId = 'wolfspeed-site-halo-inner';
    const haloOuterLayerId = 'wolfspeed-site-halo-outer';
    const sparkleLayerId = 'wolfspeed-site-sparkles';

    addOrUpdateSource(m, circleSourceId, createCircleFeature(WOLFSPEED_CENTER, WOLFSPEED_RADIUS_METERS));
    addOrUpdateSource(m, haloInnerSourceId, createCircleFeature(WOLFSPEED_CENTER, WOLFSPEED_RADIUS_METERS * 0.82));
    addOrUpdateSource(m, haloOuterSourceId, createCircleFeature(WOLFSPEED_CENTER, WOLFSPEED_RADIUS_METERS * 1.28));
    addOrUpdateSource(m, sparkleSourceId, {
      type: 'FeatureCollection',
      features: createSparkleFeatures(WOLFSPEED_CENTER, WOLFSPEED_RADIUS_METERS * 1.06)
    });
    ...
    m.addLayer({
      id: 'wolfspeed-site-circle-dash-outline',
      type: 'line',
      source: circleSourceId,
      paint: {
  ...
  const removeCircleLayer = useCallback(() => {
    ...
    ['wolfspeed-site-circle-dash-outline', outlineId, haloInnerLayerId, haloOuterLayerId, sparkleLayerId, fillId].forEach(id => safeRemove(m, id));
    [sourceId, WOLFSPEED_RING_IDS.haloInnerSource, WOLFSPEED_RING_IDS.haloOuterSource, WOLFSPEED_RING_IDS.sparkleSource].forEach(id => safeRemove(m, id, false));
  ...
  const handleCleanup = useCallback(() => {
    ...
    removeCircleLayer();
    emitTimelinePause();
  }, [stopPlayback, removeChangeLayers, removeCircleLayer, emitTimelinePause]);
  ...
  useEffect(() => {
    ...
    return () => {
      if (typeof window !== 'undefined' && window.wolfspeedAnimationRef) {
        delete window.wolfspeedAnimationRef;
      }
    };
  }, [handlePlayPause, handleRestart, handleCleanup, periodData, currentIndex, isPlaying]);

  useEffect(() => {
    if (!visible || !periodData.length) return;
    const normalizedIndex = ((currentIndex % periodData.length) + periodData.length) % periodData.length;
    const period = periodData[normalizedIndex];
    emitTimelinePlayback(period, normalizedIndex);
  }, [visible, periodData, currentIndex, emitTimelinePlayback]);

  useEffect(() => {
    if (visible && isPlaying) return;
    emitTimelinePause();
  }, [visible, isPlaying, emitTimelinePause]);
  ...
  useEffect(() => {
    ...
    const loadFrames = async () => {
      try {
        const responses = await Promise.all(
          PERIODS.map(async (period) => {
            try {
              ...
              console.warn('⚡ Wolfspeed CTA: Failed loading change frame', {
                file: period.file,
                error
              });
              return null;
        ...
        console.error('⚡ Wolfspeed CTA: Unexpected error loading animation frames', error);
    ...
    if (data) {
      ...
      if (!m.getLayer(LAYER_IDS.siteLabel)) {
        m.addLayer({
          ...
          type: 'symbol',
          source: LAYER_IDS.source,
          filter: ['==', ['get', 'zone'], 'wolfspeed_site'],
          layout: {
...
export default WolfspeedChangeAnimation;
