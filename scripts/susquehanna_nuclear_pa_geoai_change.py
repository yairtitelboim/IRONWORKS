#!/usr/bin/env python3
"""
Generate Sentinel-2 change GeoJSON layers for the Susquehanna Steam Electric Station AOI (Pennsylvania).

This mirrors the Harris NC exporter but uses a Susquehanna-centric AOI and
PA-specific site metadata.

Outputs (by default):
  public/data/susquehanna_nuclear_pa/susquehanna_nuclear_pa_<year1>_<year2>.geojson
  public/data/susquehanna_nuclear_pa/susquehanna_nuclear_pa_<year1>_<year2>_stats.json
"""
...
SITE_ID = "susquehanna_nuclear_pa"
SITE_NAME = "Susquehanna Steam Electric Station"
# Berwick, PA (lng, lat) – matches ncPowerSites coordinates
SITE_CENTER = (-76.1500, 41.1000)
# Regional AOI for change detection. Use a slightly tighter radius so we
# emphasize land use change most likely coupled to the plant and corridors.
...
CORE_RADIUS_KM = 6.0
SHELL_RADIUS_KM = 12.0

# Thresholds tuned for Susquehanna's landscape; a bit lower than TMI to keep
# a few meaningful islands visible.
CORE_MIN_LOSS_HA = 12.0
SHELL_MIN_LOSS_HA = 40.0
CORE_MIN_GAIN_HA = 6.0
SHELL_MIN_GAIN_HA = 20.0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Susquehanna Nuclear PA change exporter")
    parser.add_argument(
        "--output-dir",
