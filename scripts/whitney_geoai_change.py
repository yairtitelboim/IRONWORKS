#!/usr/bin/env python3
"""
Generate Whitney (CyrusOne DFW7) land-use change GeoJSON layers.

This is a direct adaptation of the Lucid Motors agriculture change exporter that was
originally tuned for Pinal County, AZ.

Outputs:
  public/data/whitney/whitney_dfw7_<year1>_<year2>.geojson
  public/data/whitney/whitney_dfw7_<year1>_<year2>_stats.json
"""
...
SITE_ID = "whitney_dfw7"
SITE_NAME = "CyrusOne DFW7 Data Center"
SITE_CENTER = (-97.347, 31.9315)  # (lng, lat)
SITE_RADIUS_METERS = 4500  # Covers construction footprint + supporting infrastructure
...
def feature_collection_from_change(change_image, geometry, scale: int = 20):
    ...
    # Limit feature count to avoid Earth Engine 5000-element query cap.
    vectors = vectors.limit(5000)
    center = ee.Geometry.Point(SITE_CENTER)
...
def process_period(
    ...
    output_geojson = os.path.join(
        out_dir, f"{SITE_ID}_{year_start}_{year_end}.geojson"
    )
    if not overwrite and os.path.exists(output_geojson):
        print(
            f"✅ {output_geojson} exists, skipping (use --overwrite to regenerate)"
        )
        return
    ...
    output_stats = os.path.join(
        out_dir, f"{SITE_ID}_{year_start}_{year_end}_stats.json"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Whitney (DFW7) land-use change exporter"
    )
    parser.add_argument(
        "--out-dir",
        default="public/data/whitney",
        help="Output directory for GeoJSON files",
    )
    parser.add_argument(
        "--pairs",
        type=parse_pairs,
        help="Year pairs as comma-separated list (e.g. 2022-2023,2023-2024)",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    if not initialize_gee():
        raise SystemExit("Failed to initialize Google Earth Engine credentials")
    ...
    print("✅ Completed Whitney exports")
