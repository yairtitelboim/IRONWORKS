...
const SITE_KEY = 'toyota_battery_nc';
...
// Water-aware color ramps: vary water_change color by area_ha
...
const ToyotaBatteryChangeAnimation = ({ map, data, changeData = null, visible = true }) => {
  ...
  const addCircleLayer = useCallback(() => {
    ...
    m.addLayer({
      id: 'toyota-site-circle-dash-outline',
      type: 'line',
      source: circleSourceId,
      paint: {
  ...
  const removeCircleLayer = useCallback(() => {
    ...
    ['toyota-site-circle-dash-outline', outlineId, haloInnerLayerId, haloOuterLayerId, sparkleLayerId, fillId].forEach(id => safeRemove(m, id));
    [sourceId, TOYOTA_RING_IDS.haloInnerSource, TOYOTA_RING_IDS.haloOuterSource, TOYOTA_RING_IDS.sparkleSource].forEach(id => safeRemove(m, id, false));
  ...
  useEffect(() => {
    ...
    const loadFrames = async () => {
      try {
        const responses = await Promise.all(
          PERIODS.map(async (period) => {
            try {
              ...
              console.warn('🚗 Toyota CTA: Failed loading change frame', {
                file: period.file,
                error
              });
              return null;
        ...
        console.error('🚗 Toyota CTA: Unexpected error loading animation frames', error);
