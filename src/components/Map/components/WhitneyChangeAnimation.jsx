...
const PERIODS = [
  { start: 2019, end: 2020, label: '2019 → 2020', file: '/data/whitney/whitney_dfw7_2019_2020.geojson' },
  { start: 2020, end: 2021, label: '2020 → 2021', file: '/data/whitney/whitney_dfw7_2020_2021.geojson' },
  { start: 2021, end: 2022, label: '2021 → 2022', file: '/data/whitney/whitney_dfw7_2021_2022.geojson' },
  { start: 2022, end: 2023, label: '2022 → 2023', file: '/data/whitney/whitney_dfw7_2022_2023.geojson' },
  { start: 2023, end: 2024, label: '2023 → 2024', file: '/data/whitney/whitney_dfw7_2023_2024.geojson' },
  { start: 2024, end: 2025, label: '2024 → 2025', file: '/data/whitney/whitney_dfw7_2024_2025.geojson' }
];

const WHITNEY_CENTER = [-97.347, 31.9315];
const WHITNEY_RADIUS_METERS = 4500;
...
const createCircleCoords = (radiusMeters, steps = 256) => {
  ...
  const centerLatRad = (WHITNEY_CENTER[1] * Math.PI) / 180;
  const centerLngRad = (WHITNEY_CENTER[0] * Math.PI) / 180;
...
const WhitneyChangeAnimation = ({ map, onClose }) => {
  ...
  const animate = () => {
      ...
      const fillColor = 'rgba(56, 189, 248, 0.24)';
      const outlineColor = 'rgba(56, 189, 248, 0.52)';
      const lineColor = 'rgba(37, 99, 235, 0.78)';
      ...
      const innerOpacity = 0.22;
      const outerOpacity = 0.12;
      ...
      if (sparkleLayerId && mapInstance.getLayer(sparkleLayerId)) {
        mapInstance.setPaintProperty(sparkleLayerId, 'circle-opacity', 0.08);
        mapInstance.setPaintProperty(sparkleLayerId, 'circle-radius', 3.2);
      }
  ...
  const animateLayerIn = useCallback((fillId, config) => {
    ...
    const step = (now) => {
      ...
      if (progress < 1) {
        ...
        applyTimelineLegendHighlight(timelineHighlightKey, fillId, mapInstance);
      }
    };
  ...
  const animateOutlineGlow = useCallback((outlineId) => {
    ...
    const step = (now) => {
      ...
      if (elapsed >= totalDuration) {
        ...
        applyTimelineLegendHighlight(timelineHighlightKey, null, mapInstance, outlineId);
  ...
  const addCalloutLayers = useCallback((callouts) => {
    ...
    const sourceId = 'whitney-callouts-source';
    const layerId = 'whitney-callouts-labels';
  ...
  const addCircleLayer = useCallback(() => {
    ...
    const circleSourceId = 'whitney-site-circle-source';
    const haloInnerSourceId = 'whitney-site-halo-inner-source';
    const haloOuterSourceId = 'whitney-site-halo-outer-source';
    const sparkleSourceId = 'whitney-site-sparkles-source';

    const fillId = 'whitney-site-circle-fill';
    const outlineId = 'whitney-site-circle-outline';
    const haloInnerLayerId = 'whitney-site-halo-inner';
    const haloOuterLayerId = 'whitney-site-halo-outer';
    const sparkleLayerId = 'whitney-site-sparkles';

    const circleCoords = createCircleCoords(WHITNEY_RADIUS_METERS);
    const haloInnerCoords = createCircleCoords(WHITNEY_RADIUS_METERS * 0.85);
    const haloOuterCoords = createCircleCoords(WHITNEY_RADIUS_METERS * 1.32);
    const sparkleFeatures = createSparkleFeatures(WHITNEY_RADIUS_METERS * 1.05, 16);
    ...
    mapInstance.addLayer({
      ...
      paint: {
        'fill-color': 'rgba(56, 189, 248, 0.32)',
        'fill-opacity': 0,
        'fill-outline-color': 'rgba(37, 99, 235, 0)'
      }
    });
    ...
    mapInstance.addLayer({
      ...
      paint: {
        'fill-color': 'rgba(56, 189, 248, 0.16)',
    ...
    mapInstance.addLayer({
      ...
      source: circleSourceId,
      paint: {
        'line-color': 'rgba(37, 99, 235, 0.65)',
        'line-width': 2,
  ...
  const removeAddedLayers = useCallback(() => {
    ...
    // Remove all layers that start with 'whitney-change-' pattern
    const allLayers = mapInstance.getStyle().layers;
    allLayers.forEach(layer => {
      if (layer.id.startsWith('whitney-change-')) {
    ...
    // Remove all sources that start with 'whitney-change-' pattern
    const allSources = Object.keys(mapInstance.getStyle().sources);
    allSources.forEach(sourceId => {
      if (sourceId.startsWith('whitney-change-')) {
        if (mapInstance.getSource(sourceId)) {
          mapInstance.removeSource(sourceId);
        }
      }
...
const stopPlayback = useCallback(() => {
  ...
  const loadPeriodFiles = useCallback(async () => {
    ...
    for (const period of PERIODS) {
      try {
        ...
        if (!response.ok) {
          console.warn(`WhitneyChange: missing ${period.file} (${response.status})`);
          continue;
        }
        const geojson = await response.json();
        loaded.push({ ...period, geojson });
      } catch (error) {
        console.warn(`WhitneyChange: failed to load ${period.file}`, error);
      }
    }
    if (!mountedRef.current) return;
  ...
  const addPeriodToMap = useCallback((period, periodIndex = 0) => {
    ...
    const sourceId = `whitney-change-${period.start}-${period.end}-source`;
  ...
  useEffect(() => {
    ...
    return () => {
      window.whitneyAnimationRef = null;
    };
...
export default WhitneyChangeAnimation;
