...
export class OsmTool {
  ...
  processVisualNode(element, features, visualLayers) {
    ...
    // Check if this is a major university by name (location-aware)
    ...
    const isRice = name.includes('rice') ||
                   name.includes('rice university') ||
                   name.includes('rice ') ||
                   name.includes('rice-') ||
                   name.includes('rice/');
    const isUH = name.includes('university of houston') ||
                 name.includes('uh ') ||
                 name.includes('uh-') ||
                 name.includes('uh/') ||
                 (name.includes('houston') && name.includes('university') && !name.includes('baptist'));
    const isTSU = name.includes('texas southern') ||
                  name.includes('tsu ') ||
                  name.includes('tsu-') ||
                  name.includes('tsu/');
    const isHBU = name.includes('houston baptist') ||
                  name.includes('hbu ') ||
                  name.includes('hbu-') ||
                  name.includes('hbu/') ||
                  (name.includes('houston') && name.includes('baptist'));

    // Calculate if this is a major university (any location)
    isMajorUniversity = isMIT || isHarvard || isNortheastern || isBostonUniversity || isTufts ||
                       isRice || isUH || isTSU || isHBU;
      ...
      const feature = {
        ...
        properties: {
          ...
          university_type: isMIT ? 'MIT' : 
                         ...
                         isTufts ? 'Tufts' :
                         isRice ? 'Rice University' :
                         isUH ? 'University of Houston' :
                         isTSU ? 'Texas Southern University' :
                         isHBU ? 'Houston Baptist University' : 'Other'
        }
      };
  ...
  async addCachedLayersToMap(map, visualData, lat, lng) {
    // Remove existing layers (but preserve neighborhood boundaries)
    const layersToRemove = [
      ...
      'neighborhood-boundaries-highlight', 'neighborhood-boundaries-highlight-fill'
    ];
    ...
    // Don't remove neighborhood-boundaries source - keep it for click detection
    ...
    if (!map.current.getSource('neighborhood-boundaries')) {
      await this.loadNeighborhoodBoundaries(map);
    } else if (!map.current.getLayer('neighborhood-boundaries-fill')) {
      // Source exists but layer doesn't - add the layer
      ...
      // Add visible line layer for neighborhood boundaries
      map.current.addLayer({
      ...
      console.log('✅ Neighborhood boundaries click layer and stroke added');
    }
    ...
    const layerLoadingSequence = [
      ...
      { id: 'osm-rice', name: 'Rice University', delay: 750 },
      { id: 'osm-uh', name: 'University of Houston', delay: 900 },
      { id: 'osm-tsu', name: 'Texas Southern University', delay: 1050 },
      { id: 'osm-hbu', name: 'Houston Baptist University', delay: 1200 },
      { id: 'osm-other-universities', name: 'Other Universities', delay: 1350 },
      { id: 'osm-offices', name: 'Office Buildings', delay: 1500 },
      { id: 'osm-transportation-stations', name: 'Transportation Stations', delay: 1650 },
      { id: 'osm-parks', name: 'Parks', delay: 1800 },
      { id: 'osm-water-lines', name: 'Water Features (Lines)', delay: 1950 },
      { id: 'osm-water-fill', name: 'Water Features (Areas)', delay: 2100 },
      { id: 'osm-commercial', name: 'Commercial Zones', delay: 2250 },
      { id: 'osm-critical', name: 'Critical Infrastructure', delay: 2400 },
      { id: 'osm-radius', name: 'Analysis Radius', delay: 2550 }
    ];
  ...
  async addVisualLayersToMap(map, visualData, lat, lng) {
    ...
    // Ensure neighborhood boundaries are loaded and layer exists
    if (!map.current.getSource('neighborhood-boundaries')) {
      await this.loadNeighborhoodBoundaries(map);
    } else if (!map.current.getLayer('neighborhood-boundaries-fill')) {
      // Source exists but layer doesn't - add the layer
  ...
  async loadNeighborhoodBoundaries(map) {
    try {
      ...
      // Fetch the Houston Super Neighborhoods data
      const response = await fetch('/Listings/TLDR/COH_Super_Neighborhoods_view_-959162351783753804.geojson');
      if (!response.ok) {
        throw new Error(`Failed to load neighborhood data: ${response.status}`);
      }
      
      const neighborhoodData = await response.json();
      console.log(`📊 Loaded ${neighborhoodData.features?.length || 0} neighborhood boundaries`);
      
      // Use the data as-is since it's already in WGS84 format
      const convertedFeatures = neighborhoodData.features.map(feature => {
        return {
          ...feature,
          properties: {
            ...feature.properties,
            neighborhood: feature.properties.SNBNAME || 'Unknown'
          }
        };
      });
      ...
      // Add a visible line layer for neighborhood boundaries
      map.current.addLayer({
        id: 'neighborhood-boundaries-line',
        type: 'line',
        source: 'neighborhood-boundaries',
      ...
      console.log('✅ Neighborhood boundaries click layer and stroke added to map');
  ...
  /**
   * Ensure neighborhood boundaries layer exists for click detection
   */
  ensureNeighborhoodBoundariesLayer(map) {
    if (!map?.current) return false;
    
    // Check if source exists
    if (!map.current.getSource('neighborhood-boundaries')) {
      console.log('⚠️ Neighborhood boundaries source not found, loading...');
      this.loadNeighborhoodBoundaries(map);
      return false;
    }
    
    // Check if layer exists
    if (!map.current.getLayer('neighborhood-boundaries-fill')) {
      console.log('⚠️ Neighborhood boundaries layer not found, adding...');
      map.current.addLayer({
        id: 'neighborhood-boundaries-fill',
        type: 'fill',
        source: 'neighborhood-boundaries',
  ...
  setupNeighborhoodClickHandler(map) {
    ...
    // Ensure the layer exists before setting up handlers
    this.ensureNeighborhoodBoundariesLayer(map);
    ...
    map.current.on('click', (e) => {
      ...
      // Ensure layer exists before querying
      if (!this.ensureNeighborhoodBoundariesLayer(map)) {
        return;
      }
      
      // Query for neighborhood features specifically
      const neighborhoodFeatures = map.current.queryRenderedFeatures(e.point, {
        layers: ['neighborhood-boundaries-fill']
      });
  ...
  addSingleLayer(map, layerId, lat, lng) {
    ...
    map.current.addLayer({
      ...
      filter: ['all',
        ...
        ['==', ['get', 'university_type'], 'Rice University']
      ],
      paint: {
            'circle-radius': 12,
            'circle-color': '#dc2626',
    ...
    map.current.addLayer({
      ...
      filter: ['all',
        ...
        ['==', ['get', 'university_type'], 'University of Houston']
      ],
      paint: {
            'circle-radius': 10,
            'circle-color': '#7c2d12',
    ...
    map.current.addLayer({
      ...
      filter: ['all',
        ...
        ['==', ['get', 'university_type'], 'Texas Southern University']
      ],
      paint: {
            'circle-radius': 8,
            'circle-color': '#ea580c',
    ...
    map.current.addLayer({
      ...
      filter: ['all',
        ...
        ['==', ['get', 'university_type'], 'Houston Baptist University']
      ],
      paint: {
            'circle-radius': 8,
            'circle-color': '#0891b2',
