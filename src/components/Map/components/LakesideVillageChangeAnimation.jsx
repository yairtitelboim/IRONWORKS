...
const PERIODS = [
  { start: 2019, end: 2020, label: '2019 → 2020', file: '/data/lakeside_village/lakeside_village_circle_2019_2020.geojson' },
  { start: 2020, end: 2021, label: '2020 → 2021', file: '/data/lakeside_village/lakeside_village_circle_2020_2021.geojson' },
  { start: 2021, end: 2022, label: '2021 → 2022', file: '/data/lakeside_village/lakeside_village_circle_2021_2022.geojson' },
  { start: 2022, end: 2023, label: '2022 → 2023', file: '/data/lakeside_village/lakeside_village_circle_2022_2023.geojson' },
  { start: 2023, end: 2024, label: '2023 → 2024', file: '/data/lakeside_village/lakeside_village_circle_2023_2024.geojson' },
  { start: 2024, end: 2025, label: '2024 → 2025', file: '/data/lakeside_village/lakeside_village_circle_2024_2025.geojson' }
];

const LAKESIDE_CENTER = [-97.405, 31.98];
const LAKESIDE_RADIUS_METERS = 4500;
...
const SITE_KEY = 'lakeside_village';
...
const createCircleCoords = (radiusMeters, steps = 256) => {
  ...
  const centerLatRad = (LAKESIDE_CENTER[1] * Math.PI) / 180;
  const centerLngRad = (LAKESIDE_CENTER[0] * Math.PI) / 180;
...
const LakesideVillageChangeAnimation = ({ map, onClose }) => {
  ...
  const animate = () => {
      ...
      const fillColor = 'rgba(59, 130, 246, 0.22)';
      const outlineColor = 'rgba(56, 189, 248, 0.48)';
      const lineColor = 'rgba(56, 189, 248, 0.75)';
  ...
  const addCalloutLayers = useCallback((callouts) => {
    ...
    const sourceId = 'lakeside_village-callouts-source';
    const layerId = 'lakeside_village-callouts-labels';
  ...
  const addCircleLayer = useCallback(() => {
    ...
    const circleSourceId = 'lakeside_village-site-circle-source';
    const haloInnerSourceId = 'lakeside_village-site-halo-inner-source';
    const haloOuterSourceId = 'lakeside_village-site-halo-outer-source';
    const sparkleSourceId = 'lakeside_village-site-sparkles-source';

    const fillId = 'lakeside_village-site-circle-fill';
    const outlineId = 'lakeside_village-site-circle-outline';
    const haloInnerLayerId = 'lakeside_village-site-halo-inner';
    const haloOuterLayerId = 'lakeside_village-site-halo-outer';
    const sparkleLayerId = 'lakeside_village-site-sparkles';

    const circleCoords = createCircleCoords(LAKESIDE_RADIUS_METERS);
    const haloInnerCoords = createCircleCoords(LAKESIDE_RADIUS_METERS * 0.85);
    const haloOuterCoords = createCircleCoords(LAKESIDE_RADIUS_METERS * 1.32);
    const sparkleFeatures = createSparkleFeatures(LAKESIDE_RADIUS_METERS * 1.05, 16);
    ...
    mapInstance.addLayer({
      ...
      paint: {
        ...
        'fill-opacity': 0,
        'fill-outline-color': 'rgba(56, 189, 248, 0)'
      }
    });
    ...
    mapInstance.addLayer({
      ...
      paint: {
        'fill-color': 'rgba(59, 130, 246, 0.26)',
        'fill-opacity': 0,
        'fill-outline-color': 'rgba(56, 189, 248, 0)'
      }
    });
    ...
    mapInstance.addLayer({
      ...
      paint: {
        ...
        'fill-outline-color': 'rgba(56, 189, 248, 0.28)'
      }
    });
    ...
    mapInstance.addLayer({
      ...
      source: circleSourceId,
      paint: {
        'line-color': 'rgba(56, 189, 248, 0.58)',
        'line-width': 2,
  ...
  const removeAddedLayers = useCallback(() => {
    ...
    // Remove all layers that start with 'lakeside_village-change-' pattern
    const allLayers = mapInstance.getStyle().layers;
    allLayers.forEach(layer => {
      if (layer.id.startsWith('lakeside_village-change-')) {
    ...
    // Remove all sources that start with 'lakeside_village-change-' pattern
    const allSources = Object.keys(mapInstance.getStyle().sources);
    allSources.forEach(sourceId => {
      if (sourceId.startsWith('lakeside_village-change-')) {
        if (mapInstance.getSource(sourceId)) {
          mapInstance.removeSource(sourceId);
        }
      }
...
const stopPlayback = useCallback(() => {
  ...
  const loadPeriodFiles = useCallback(async () => {
    ...
    for (const period of PERIODS) {
      try {
        ...
        if (!response.ok) {
          console.warn(`LakesideVillageChange: missing ${period.file} (${response.status})`);
          continue;
        }
        const geojson = await response.json();
        loaded.push({ ...period, geojson });
      } catch (error) {
        console.warn(`LakesideVillageChange: failed to load ${period.file}`, error);
      }
    }
    if (!mountedRef.current) return;
  ...
  const addPeriodToMap = useCallback((period, periodIndex = 0) => {
    ...
    const sourceId = `lakeside_village-change-${period.start}-${period.end}-source`;
  ...
  useEffect(() => {
    window.lakesideVillageAnimationRef = {
      handlePlayPause,
      handleRestart,
      handleCleanup,
      isPlaying: () => isPlaying
    };
    console.log('🏡 Lakeside animation: controller registered');
    return () => {
      console.log('🏡 Lakeside animation: controller unregistered');
      window.lakesideVillageAnimationRef = null;
    };
...
export default LakesideVillageChangeAnimation;
